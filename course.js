...
data.forEach((content, index) => {
    let courseElement = `
        <div class="video" data-id="${content.id}">
            <img src="assets/image/play-icon-white.svg" alt="">
            <p>0${index + 1}</p>
...
contents[0].querySelector('img').src = 'assets/image/stop-icon-white.svg';
...
contents.forEach(selected_content => {
    selected_content.onclick = () => {
        ...
        for (all_contents of contents) {
            ...
            all_contents.querySelector('img').src = 'assets/image/play-icon-blue.svg';

        }

        selected_content.classList.add('active');
        selected_content.querySelector('img').src = 'assets/image/stop-icon-white.svg';

        let match_content = data.find(content => content.id == selected_content.dataset.id);
        content.src = 'assets/video/' + match_content.name;
